...
A local Streamlit dashboard that enriches a Power BI engagement report with two AI-performance lenses, giving content editors a unified view of how articles perform for both human readers and AI retrieval systems.
...
|--------|-----------------|--------|
| **AI Readiness** | How well an article is structured for RAG retrieval — evaluated across 10 dimensions (heading hierarchy, chunk autonomy, context completeness, etc.) by an LLM | **High / Medium / Low** band |
...
1. **Data Table** — Filterable, sortable table with all original columns plus the two new scores
...
All pipeline steps are **incremental** — re-running skips already-processed articles.
...
- Python 3.11+
- Azure OpenAI resource with a `gpt-5-mini` deployment (for AI Readiness grading and chat)
...
| `docs/progress.txt` | What has been built and what remains |
| `docs/implementation_plans/content-performance-dashboard.md` | Original detailed implementation spec |
